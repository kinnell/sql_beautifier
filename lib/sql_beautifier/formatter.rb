...
module SqlBeautifier
  class Formatter
    ...
    def extract_leading_sentinels!
      ...
      while remaining.match?(%r{\A#{CommentStripper::SENTINEL_PATTERN}[[:space:]]*})
        match = remaining.match(%r{\A(#{CommentStripper::SENTINEL_PATTERN}[[:space:]]*)})
        leading << match[1]
        remaining = remaining[match[1].length..]
      end

      @normalized_value = remaining
      leading
    end
