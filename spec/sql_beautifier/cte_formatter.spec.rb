...
RSpec.describe SqlBeautifier::CteQuery do
  describe ".parse" do
    ...
    context "with a single CTE definition" do
      let(:cte_query) { described_class.parse("with cte as (select id from users) select * from cte") }
      ...
      it "parses the CTE name" do
        expect(cte_query.definitions.first.name).to eq("cte")
      end
      ...
      it "parses the main query" do
        expect(cte_query.main_query_sql).to eq("select * from cte")
      end
    end
    ...
    context "with a recursive CTE" do
      let(:cte_query) { described_class.parse("with recursive cte as (select 1 union all select n + 1 from cte where n < 10) select * from cte") }
    ...
    context "with a CTE column list" do
      let(:cte_query) { described_class.parse("with cte(a, b) as (select 1, 2) select * from cte") }
    ...
    context "with a materialized CTE" do
      let(:cte_query) { described_class.parse("with cte as materialized (select 1) select * from cte") }
    ...
    context "with a not materialized CTE" do
      let(:cte_query) { described_class.parse("with cte as not materialized (select 1) select * from cte") }
    ...
    context "with nested parentheses in the CTE body" do
      let(:cte_query) { described_class.parse("with cte as (select count(id) from users where active = true) select * from cte") }
    ...
    context "with string literals containing parentheses in the CTE body" do
      let(:cte_query) { described_class.parse("with cte as (select id from users where name = 'O''Brien') select * from cte") }
  ...
  describe "#render" do
    ...
    context "with a CTE column list" do
      let(:output) { described_class.parse("with cte (a, b) as (select 1, 2) select * from cte").render }
    ...
    context "with a materialized CTE" do
      let(:output) { described_class.parse("with cte as materialized (select id from users) select * from cte").render }
    ...
    context "with a not materialized CTE" do
      let(:output) { described_class.parse("with cte as not materialized (select id from users) select * from cte").render }
